...
//
//   parity <backup.json> [--today 2026-09-21]  > answers.json
//
...
func usage(_ code: Int32) -> Never {
    FileHandle.standardError.write(Data("usage: parity <backup.json> [--today YYYY-MM-DD] > answers.json\n".utf8))
    exit(code)
}
...
do {
    let parity = try Parity(backupPath: backupPath, today: today)
